		...
		<view class="login-content">
			...
			<view>
				<app-hotspot :hotspot="auth_page.hotspot">
					...
					<!-- #endif -->
					<!-- #ifdef MP-WEIXIN -->
					<button class="login-btn" scope="userInfo" @getAuthorize="getUserInfo"
						@getuserProfile="getUserProfile" @click="getUserProfileClick">
					</button>
					<!-- #endif -->
					<!-- #ifdef H5 -->
...
<script>
	...
	export default {
		...
		methods: {
			getUserProfileClick(e,userinfo) {
				console.log(userinfo,'cs-login');
				if(!userinfo){
					uni.navigateTo({
						url: '/plugins/newuser/index/index'
					})
					return;
				}
				// #ifdef MP
